# 再発防止メモ (Terraform / AWS)

このメモは、次の再発しやすい問題を減らし、発生時に迅速に復旧するための運用手順をまとめたものです。

- IAM 伝播遅延
- KMS キー状態の異常
- Secrets Manager の import / state 不整合
...
- Terraform の依存関係が正しくても、AWS IAM の反映は結果整合性のため即時ではない。
- API 上は作成済みでも、連携先サービスからすぐに利用できないことがある。

### 1.3. 予防策

- 作成時に権限が必要なリソースには、明示的に depends_on を設定する。
- とくに aws_iam_role_policy_attachment への依存を明確にする。

### 1.4. 運用時の対処

1. 権限系エラーで初回 apply が失敗し、設定自体が正しい場合は 30-120 秒待つ。
...
- Secrets Manager、SSM SecureString、各種サービス連携で暗号化/復号が失敗する。
...
- KMS ポリシーの principal に IAM グループ ARN は使わない。
...
- AWS 側に既存シークレットがあるのに、Terraform が新規作成しようとして失敗する。
